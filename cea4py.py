...
if __name__ == '__main__': # テスト用．これ以降がなくても他ファイルからモジュールとして関数は呼べる
    ...
    # ---- パラメータ指定 ----
    pc_array = np.linspace(1, 7, 4) # 適っ当にPc=1, 3, 5, 7MPa
    ar_array = np.linspace(2, 10, 50) # ARは適っ当に2から10までを50分割してみる
    vsar_data_array = np.zeros((len(ar_array), 6))
    ...
    for Pc in pc_array:
        ...
        # ---- PLOT ----
        plt.plot(ar_array, Isp, label='Pc=%.1f' % (Pc))
        # ----      ----
    ...
    plt.ylabel('Isp (sec)')
    plt.grid()
    plt.title('LOX/C2H5OH 100% frozen nfz=3')
